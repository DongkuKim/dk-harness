...
- `scripts/release-pack-check.mjs`: release gate that validates the packed package and composes every curated preset from the packaged CLI.
...
- `pnpm run fixtures:sync` regenerates `scaffolds/generated/` from the module catalog plus preset manifests.
- `package.json` publishes the CLI and module catalog in the package tarball. Committed generated fixtures stay in the repo and are not shipped.
...
- `package-nextjs`: repeatable publishable Next.js UI kit package module that emits `packages/<id>`.
...
- Preset manifests in `scaffolds/presets/` describe high-value supported compositions such as web-only, publishable UI kit package, web plus FastAPI, web plus Axum, mixed-runtime, and repeated-backend stacks.
...
- Scaffold evaluation packs the package artifact, composes each curated preset with the packaged CLI, installs its dependencies, and runs generated repo checks.
